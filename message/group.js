...
let welnong = `Selamat datang member baru`
           let bug = [
            {buttonId: 'beban', buttonText: {displayText: 'Waduh Ada Beban😁'}, type: 1},
            {buttonId: 'desk', buttonText: {displayText: 'Deskripsi Grup'}, type: 1}
            ]
...
Babay @${anj.participants[0].split("@")[0]} 👋 selamat kluar Dari ${groupName}  ,Semoga Di terima Di sisi Allah`
let btn = `Selamat datang member baru`
           let anjay = [
            {buttonId: 'nice', buttonText: {displayText: 'Tombol To Isekai'}, type: 1}
            ]
            sendButImage(anj.jid, wetong, btn, await getBuffer(pic), anjay, {contextInfo:{"mentionedJid":[anj.participants[0]]}})
